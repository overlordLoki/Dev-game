...
namespace Ashenveil.Core.Levels
{
    ...
    public static class MapLoader
    {
        public static LoadedMap Load(string name)
        {
            ...
            // objects: data -> real object instances. The (type, variety) -> class
            // mapping lives in ObjectFactory; add new placeables there, not here.
            var objects = new List<IObject>();
            if (data.objects != null)
            {
                foreach (var o in data.objects)
                {
                    var obj = ObjectFactory.Create(o);
